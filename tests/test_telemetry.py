...
class TestCostTracker:
    ...
    def test_record_call_basic(self):
        tracker = CostTracker()
        tracker.record_call(
            operation="describe_item", provider="openai", model="gpt-4o",
            input_text="Hello world", output_text="Hello world description",
    ...
    def test_record_call_failure(self):
        tracker = CostTracker()
        tracker.record_call(
            operation="embed", provider="anthropic", model="claude-3-haiku-20240307",
            input_text="Test text", latency_ms=50.0, success=False
        )
        assert len(tracker._records) == 1
        record = tracker._records[0]
    ...
    def test_get_records_since(self):
        ...
        old_record = CostRecord(
            ...
            cost_usd=0.001, latency_ms=100.0, success=True
        )
        ...
        recent_record = CostRecord(
            ...
            cost_usd=0.0005, latency_ms=50.0, success=True
        )
    ...
    def test_get_total_cost(self):
        tracker = CostTracker()
        record1 = CostRecord(
            timestamp=datetime.now(timezone.utc), operation="describe_item",
            provider="openai", model="gpt-4o", input_tokens=100, output_tokens=50,
            total_tokens=150, cost_usd=0.001, latency_ms=100.0, success=True
        )
        record2 = CostRecord(
            timestamp=datetime.now(timezone.utc), operation="embed",
            provider="openai", model="text-embedding-3-small", input_tokens=200,
            output_tokens=0, total_tokens=200, cost_usd=0.0005, latency_ms=50.0, success=True
        )
        tracker._records = [record1, record2]
        total_cost = tracker.get_total_cost(hours=24)
        assert abs(total_cost - 0.0015) < 1e-6
    ...
    def test_get_call_count(self):
        ...
        record2 = CostRecord(
            timestamp=datetime.now(timezone.utc), operation="embed",
            provider="openai", model="text-embedding-3-small", input_tokens=200,
            output_tokens=0, total_tokens=200, cost_usd=0.0005, latency_ms=50.0, success=False
        )
        tracker._records = [record1, record2]
    ...
    def test_record_cost_function(self):
        with patch('src.telemetry.cost._cost_tracker') as mock_tracker:
            record_cost(
                operation="describe_item", latency_seconds=1.5, success=True,
                provider="google", model="gemini-1.5-pro",
                input_text="Test input", output_text="Test output"
            )
            mock_tracker.record_call.assert_called_once()
            _args, kwargs = mock_tracker.record_call.call_args
            ...
            assert kwargs["latency_ms"] == 1500.0
    ...
    def test_get_cost_report_with_data(self):
        with patch('src.telemetry.cost._cost_tracker') as mock_tracker:
            ...
            mock_tracker.get_token_usage.return_value = {
                "input_tokens": 300, "output_tokens": 50, "total_tokens": 350
            }
...
class TestTracing:
    ...
    def test_trace_functions_noop_when_otel_not_available(self):
        trace_ai_call(operation="describe_item", provider="openai", model="gpt-4o", latency=1.0, status="success")
        trace_describe_item(image_path="/test/image.jpg", user_text="test", latency=1.0, success=True)
        trace_embed(text="test text", latency=0.5, success=True)
    ...
    def test_trace_ai_call_sets_attributes(self):
        with patch('src.telemetry.tracing.get_tracer') as mock_get_tracer:
            ...
            trace_ai_call(
                operation="describe_item", provider="anthropic",
                model="claude-3-5-sonnet-20241022", latency=1.5, status="success",
                text_length=10, result_confidence=0.85
            )
    ...
    def test_trace_describe_item_integration(self):
        with patch('src.telemetry.tracing.trace_ai_call') as mock_trace:
            trace_describe_item(
                image_path="/lost/umbrella.jpg", user_text="Lost my black umbrella",
                latency=2.3, success=True, provider="openai", model="gpt-4o",
                result=Mock(object_class="umbrella", confidence=0.9), error=None
            )
            mock_trace.assert_called_once()
            _args, kwargs = mock_trace.call_args
            assert kwargs["operation"] == "describe_item"
    ...
    def test_trace_embed_integration(self):
        with patch('src.telemetry.tracing.trace_ai_call') as mock_trace:
            trace_embed(
                text="Black umbrella with bent rib", latency=0.8, success=True,
                provider="openai", model="text-embedding-3-small",
                embedding=[0.1, 0.2, 0.3], error=None
            )
            mock_trace.assert_called_once()
            _args, kwargs = mock_trace.call_args
    ...
    def test_trace_embed_error(self):
        with patch('src.telemetry.tracing.trace_ai_call') as mock_trace:
            trace_embed(
                text="test", latency=0.5, success=False, provider="openai",
                model="text-embedding-3-small", embedding=None, error="API timeout"
            )
            mock_trace.assert_called_once()
            _args, kwargs = mock_trace.call_args
            assert kwargs["operation"] == "embed"
