...
class CostTracker:
    ...
    def _estimate_tokens(self, text: str) -> int:
        """Rough estimation of token count from text supporting non-ASCII."""
        if not text:
            return 0
        # Учитываем, что кириллические символы берут больше токенов
        non_ascii = sum(1 for c in text if ord(c) > 127)
        ascii_len = len(text) - non_ascii
        return max(1, (ascii_len // 4) + (non_ascii // 2))
